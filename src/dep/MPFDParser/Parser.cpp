...
void MPFD::Parser::_ParseHeaders(std::string headers) {
    ...
    if (name_pos == std::string::npos) {
        ...
        if (name_end_pos == std::string::npos) {
			...
			if(CurrentField)
				delete CurrentField;// unexpected case..
			CurrentField = new Field(ProcessingFieldName);
        }

        // find filename if exists
        size_t filename_pos = headers.find("filename=\"");
        if (filename_pos == std::string::npos) {
            CurrentField->SetType(Field::TextType);
        } else {
            CurrentField->SetType(Field::FileType);
            CurrentField->SetTempDir(TempDirForFileUpload);
            CurrentField->SetUploadedFilesStorage(WhereToStoreUploadedFiles);
            ...
            if (filename_end_pos == std::string::npos) {
                ...
                CurrentField->SetFileName(filename);
            }
            ...
            if (content_type_pos != std::string::npos) {
                ...
                CurrentField->SetFileContentType(content_type);
            }
        }
    }
}
...
void MPFD::Parser::TruncateDataCollectorFromTheBeginning(long n) {
    ...
    DataCollector = new char[TruncatedDataCollectorLength+1];
	memset(DataCollector, 0, TruncatedDataCollectorLength+1);
