...
void JSONStream::push(const void *data, size_t size) json_nothrow {
	if (state){
    ...
    if (json_likely(pos != json_string::npos)){
	   ...
	   if (end != json_string::npos){
		  ...
		  START_MEM_SCOPE	 
			 ...
			 size_t remainsize = buffer.size() - end - 1;
			 #ifndef JSON_LIBRARY
				retry = call(temp, remainsize, getIdentifier());
			 #else
				retry = call(&temp, remainsize, getIdentifier());
			 #endif
		  END_MEM_SCOPE
		  json_string::iterator beginning = buffer.begin();
		  buffer.erase(beginning, beginning + end + 1);
		  if(retry)
			 parse();
	   }
	   #ifdef JSON_SAFE
